## permanent variables
PROJECT			?= github.com/gpenaud/alterconso-reminder
...
ifeq ($(type),application)
	docker-compose ${DOCKER_COMPOSE_OPTIONS} up --build alterconso-reminder
	...
	docker-compose ${DOCKER_COMPOSE_OPTIONS} logs --follow alterconso-reminder
endif

## stack - rebuild and restart alterconso-reminder
rebuild:
	docker-compose ${DOCKER_COMPOSE_OPTIONS} stop alterconso-reminder
	docker-compose ${DOCKER_COMPOSE_OPTIONS} up --build alterconso-reminder
...
## docker - build the alterconso-reminder image
.PHONY: build
build:
	docker build ${DOCKER_BUILD_ARGS} --file build/package/Dockerfile --tag alterconso-reminder:latest .

## docker - enter into the alterconso-reminder container
enter:
	docker-compose ${DOCKER_COMPOSE_OPTIONS} exec alterconso-reminder /bin/sh

cleanup:
	sudo rm -rf alterconso-reminder tmp
